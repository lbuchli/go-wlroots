...
import (
	...
	"github.com/lbuchli/go-wlroots/xkb"
)
...
func (s *Server) handleNewFrame(output wlroots.Output) {
	...
	s.renderer.Clear(&wlroots.Color{A: 0, R: 0, B: 0, G: 0})
...
func (s *Server) handleNewOutput(output wlroots.Output) {
	...
	fmt.Println("Added output", output.Name())
...
func (s *Server) handleKeyBinding(sym xkb.KeySym) bool {
	...
	case xkb.KeySymF2:
		...
		// move the next view to the front of the view list
		s.views = append(s.views[1:], nextView)
